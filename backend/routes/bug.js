...
// const { BugStatus, Priority } = pkg; // if you defined BugStatus enum in schema.prisma
...
//     const { title, description, assigneeId, dueDate, attachmentUrl, priority } = req.body;
...
const { BugStatus, BugPriority } = pkg;

const router = express.Router();

/**
 * Authentication middleware (example)
 * Assume req.user = { id, role, teamId } auth middleware se aa raha hai
 */
function requireAuth(req, res, next) {
  if (!req.user) {
    return res.status(401).json({ message: "Unauthorized" });
 ...
 * POST /bugs
 * Tester naya bug raise karega
 * Body: { title, description, assigneeId, dueDate, attachmentUrl, priority }
 */
router.post("/", requireAuth, async (req, res) => {
  try {
    const { id: userId, role, teamId } = req.user;
    ...
    // Sirf TESTER bug raise kar sake
    if (role !== "TESTER") {
      return res
        .status(403)
        .json({ message: "Only testers can raise bugs" });
    ...
    // Priority validate karo (optional)
    let bugPriority = BugPriority.MEDIUM;
    if (priority && Object.values(BugPriority).includes(priority)) {
      bugPriority = priority;
    }

    // Assignee must be a DEVELOPER in same team
    const assignee = await prisma.user.findFirst({
      where: {
        id: assigneeId,
        role: "DEVELOPER",
    ...
    if (!assignee) {
      return res.status(400).json({
        message: "Invalid assignee: must be a developer in your team",
      });
    }
    ...
    console.error("Error creating bug:", err);
    return res.status(500).json({ message: "Failed to create bug" });
  }
});

/**
 * GET /bugs/assignees/list
 * Assign tab ke liye: team ke saare developers
 */
router.get("/assignees/list", requireAuth, async (req, res) => {
  try {
    const { teamId } = req.user;
...
/**
 * GET /bugs/:id
 * Sirf reporter tester ya assigned developer access kar sakta hai
 */
router.get("/:id", requireAuth, async (req, res) => {
  try {
    const { id: userId, role } = req.user;
    ...
    console.error("Error fetching bug:", err);
    return res.status(500).json({ message: "Failed to fetch bug" });
  }
});
