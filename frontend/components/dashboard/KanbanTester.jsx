...
export default function BugKanban({ developerId }) {
    ...
    const fetchBugs = async () => {
        try {
            ...
            if (developerId && developerId !== "ALL") {
                res = await axios.get(`/api/bugs`, {
                    params: { developerId },
                });
                console.log("Fetching for developerId:", developerId);
    ...
    const handleDeleteBug = async (bugId) => {
        try {
            const response = await axios.delete(`/api/bugs/${bugId}`);

            if (response.status === 200) {
                // Close the dialog
                setSelectedBug(null);

                // Remove from local state (faster)
